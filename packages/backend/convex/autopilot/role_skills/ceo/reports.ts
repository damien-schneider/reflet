...
export const formatActivityStats = (
  ...
  const roleLines = Object.entries(activityByRole)
    .sort(([, a], [, b]) => b - a)
    .map(([role, count]) => `- ${role}: ${count} actions`)
    .join("\n");
...
RECENT ACTIVITY:
${formatActivityStats(ceoContext.activityByRole, ceoContext.recentActivityCount)}
