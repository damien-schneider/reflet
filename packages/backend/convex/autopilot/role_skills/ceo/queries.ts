...
/**
 * Build comprehensive context about product state for the CEO coordination skill.
...
export const getCEOContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const activityByRole: Record<string, number> = {};
    for (const activity of activityInRange) {
      const role = activity.role;
      activityByRole[role] = (activityByRole[role] ?? 0) + 1;
    }
...
/**
 * Get detailed context for the CEO chat — includes task titles, role states,
...
export const getDetailedCEOContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    if (config) {
      for (const { name, field } of ROLE_CONFIG_FIELDS) {
        roleStates[name] = config[field] !== false;
      }
    }
