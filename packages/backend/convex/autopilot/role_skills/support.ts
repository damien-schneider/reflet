/**
 * Support role skill — triages support conversations, drafts replies,
...
export const runSupportTriage = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const guardResult = await ctx.runQuery(
      ...
      { organizationId: args.organizationId, role: "support" }
    );
    ...
    try {
      ...
      const conversations = await ctx.runQuery(
        internal.autopilot.role_skills.support.getRecentConversations,
        { organizationId: args.organizationId }
      );
      ...
      const triage = await generateObjectWithFallback({
        ...
        systemPrompt: `You are a support triage role skill for ${org?.name ?? "the product"}.
...
export const notifyFeatureShipped = internalAction({
  ...
  handler: async (ctx, args) => {
    const recentlyCompleted = await ctx.runQuery(
      internal.autopilot.role_skills.support.getRecentlyCompletedTasks,
      { organizationId: args.organizationId }
    );
