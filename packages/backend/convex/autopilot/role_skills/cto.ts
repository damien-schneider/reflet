...
import { buildRoleSkillPrompt, CTO_SYSTEM_PROMPT } from "./prompts";
...
export const technicalSpecSchema = z.object({
  ...
  implementationPrompt: z
    .string()
    .describe(
      "Self-contained, detailed prompt for an implementation worker. Include all context, code snippets, file paths, patterns to follow, specific implementation details, and acceptance criteria. Must be actionable without additional context."
    ),
  testRequirements: z
    .array(z.string())
...
export const runCTOSpecGeneration = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const guardResult = await ctx.runQuery(
      ...
      { organizationId: args.organizationId, role: "cto" }
    );
    ...
    // Log start
    await ctx.runMutation(internal.autopilot.role_skills.cto.logCtoActivity, {
      organizationId: args.organizationId,
      taskId: args.taskId,
      level: "action",
    ...
    try {
      ...
      const repoAnalysis = await ctx.runQuery(
        internal.autopilot.role_skills.cto.getRepoAnalysisForCto,
        { organizationId: args.organizationId }
      );

      // Load coding guidelines
      const agentsMd = await ctx.runQuery(
        internal.autopilot.role_skills.cto.getAgentsMd,
        { organizationId: args.organizationId }
      );
...
Create a specification that a developer can execute without asking clarifying questions.`;
      ...
      await ctx.runMutation(
        internal.autopilot.role_skills.cto.updateTaskWithSpec,
        {
          taskId: args.taskId,
