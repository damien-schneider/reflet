/** Sales role skill — Prospecting workflow. Discovers leads from web search and structures them into the pipeline. */
...
import { buildRoleSkillPrompt, SALES_SYSTEM_PROMPT } from "./prompts";
...
const markSalesTaskStatus = async (
  ...
  if (status === "done") {
    await ctx.runMutation(internal.autopilot.task_mutations.completeRoleTask, {
      taskId,
      role: "sales",
    });
    return;
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const guardResult = await ctx.runQuery(
        ...
        { organizationId: args.organizationId, role: "sales" }
      );
      ...
      const existingLeads = await ctx.runQuery(
        internal.autopilot.role_skills.sales_queries.getLeads,
        { organizationId: args.organizationId }
      );
...
Return detailed findings about each potential lead.`,
      ...
      await ctx.scheduler.runAfter(
        0,
        internal.autopilot.role_skills.sales_prospecting
