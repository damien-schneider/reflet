 ...
 * role skills score them downstream.
 *
  ...
  queries: z
    .array(
      z.object({
        ...
        targetUseCaseTitle: z.string().nullable(),
      })
    )
    .min(1)
...
export const runCommunityDiscovery = internalAction({
  ...
  handler: async (ctx, args) => {
    const { chainState, personas, useCases }: CommunityDiscoveryContext =
      await ctx.runQuery(
        internal.autopilot.role_skills.community_discovery
    ...
    for (const match of matchResult.matches.slice(0, MAX_POSTS_TO_PERSIST)) {
      ...
      const useCaseId = match.useCaseTitle
        ...
        : undefined;
      const matchTitle = match.title ?? undefined;
