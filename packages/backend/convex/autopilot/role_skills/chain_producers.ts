 ...
 * 2. Loads upstream context (single-context, no cross-role note-passing)
...
function describeRepoAnalysisBlocker(precondition: RepoAnalysisPrecondition): {
  ...
  return {
    message:
      "Cannot produce codebase_understanding: repo analysis completed but productAnalysis text is empty",
    details: RUN_ANALYSIS_HINT,
  };
}
...
export const getRepoAnalysisPrecondition = internalQuery({
  ...
  returns: v.object({
    ...
    productAnalysisReady: v.boolean(),
  ...
  handler: async (ctx, args): Promise<RepoAnalysisPrecondition> => {
    ...
    return {
      ...
      productAnalysisReady: Boolean(integration?.productAnalysis),
      lastCompletedAt: integration?.completedAt ?? null,
    };
  },
});
...
export const getChainContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const repoAnalysisRow = await ctx.db
      .query("repoAnalysis")
      .withIndex("by_organization", (q) =>
        q.eq("organizationId", args.organizationId)
      )
      ...
      .first();
    const repoAnalysis =
      repoAnalysisRow?.status === "completed" && repoAnalysisRow.productAnalysis
        ? {
            productAnalysis: repoAnalysisRow.productAnalysis,
            completedAt: repoAnalysisRow.completedAt ?? null,
          }
        : null;
...
export const writeChainDoc = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const previousChainDocs = await ctx.db
      .query("autopilotDocuments")
      .withIndex("by_org_type", (q) =>
        q.eq("organizationId", args.organizationId).eq("type", args.type)
      )
      .collect();
    for (const doc of previousChainDocs) {
      if (
        doc.status === "archived" ||
        !doc.tags?.includes("chain") ||
        doc.reviewType !== "chain_artifact"
      ) {
        continue;
      }
...
export const produceCodebaseUnderstanding = internalAction({
  ...
  handler: async (ctx, args) => {
    const context = await ctx.runQuery(
      internal.autopilot.role_skills.chain_producers.getChainContext,
      { organizationId: args.organizationId }
    );
    ...
    const result = await generateObjectWithFallback({
      ...
      systemPrompt: `You are a senior CTO distilling a free-form repository analysis into a dense, structured understanding doc consumed by every downstream role skill. No marketing fluff. ${PRE_ANSWER}`,
      prompt: `Repository analysis (free-form markdown produced by an upstream LLM exploration):\n\n${context.repoAnalysis.productAnalysis}\n\nProduce a structured codebase_understanding document.`,
      temperature: 0,
    });

    await ctx.runMutation(
      internal.autopilot.role_skills.chain_producers.writeChainDoc,
      {
        organizationId: args.organizationId,
...
const productProfileSchema = z.object({
  ...
  pricingModel: pricingModelEnum.nullable(),
});
...
export const produceProductProfile = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(
      internal.autopilot.role_skills.chain_producers.upsertProductProfile,
      {
        organizationId: args.organizationId,
        ownerRole: "cto",
        productName: result.productName,
...
export const produceBrandVoice = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(
      internal.autopilot.role_skills.chain_producers.upsertChainKnowledgeDoc,
      {
        organizationId: args.organizationId,
        docType: "brand_voice",
        ownerRole: "cto",
...
export const produceScope = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await generateObjectWithFallback({
      models: QUALITY_MODELS,
      schema: scopeSchema,
      systemPrompt: `You are a senior CTO defining what the product is — and explicitly is NOT. Strong boundaries help downstream role skills refuse irrelevant work. ${PRE_ANSWER}`,
