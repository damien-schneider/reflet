/**
 * Validator role skill — single-purpose scoring of artifacts.
 *
...
export const runValidatorPass = internalAction({
  ...
  handler: async (ctx, args) => {
    const { docs, useCases, weights } = await ctx.runQuery(
      internal.autopilot.role_skills.validator.getPendingValidationArtifacts,
    ...
    for (const doc of docs) {
      ...
      try {
        const upstreamJson = await ctx.runQuery(
          internal.autopilot.role_skills.validator.getUpstreamContextForDoc,
        ...
        await ctx.runMutation(
          internal.autopilot.role_skills.validator.writeDocValidation,
    ...
    for (const uc of useCases) {
      try {
        const upstreamJson = await ctx.runQuery(
          internal.autopilot.role_skills.validator.getUpstreamContextForUseCase,
        ...
        await ctx.runMutation(
          internal.autopilot.role_skills.validator.writeUseCaseValidation,
