/**
 * Sales role skill — Mutations and follow-up workflow.
 *
...
export const runSalesFollowUp = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const leads = await ctx.runQuery(
      internal.autopilot.role_skills.sales_queries.getLeadsDueForFollowUp,
      { organizationId: args.organizationId }
    );
    ...
    for (const lead of leadsToProcess) {
      ...
      await ctx.runMutation(
        internal.autopilot.role_skills.sales_mutations.updateLeadFollowUp,
        {
          leadId: lead._id,
