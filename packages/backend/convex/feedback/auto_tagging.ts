...
export const startBulkAutoTagging = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(
      0,
      internal.feedback.auto_tagging.processBulkAutoTagging,
      {
        organizationId: args.organizationId,
      }
    );
...
${tagsDescription}`;
    ...
    if (selectedTagIds.length > 0) {
      await ctx.runMutation(internal.feedback.auto_tagging.applyAutoTags, {
        feedbackId: args.feedbackId,
...
export const processBulkAutoTagging = internalAction({
  ...
  handler: async (
    ...
    const jobId = await ctx.runMutation(
      internal.feedback.auto_tagging.createJob,
      {
        organizationId: args.organizationId,
    ...
    const results = await Promise.allSettled(
      untaggedIds.map((feedbackId: Id<"feedback">) =>
        ctx
          .runAction(internal.feedback.auto_tagging.processAutoTagging, {
            feedbackId,
          })
