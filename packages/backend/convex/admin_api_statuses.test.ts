...
describe("admin_api_statuses", () => {
  test("createStatus should create with auto-incremented order", async () => {
    ...
    await t.mutation(internal.admin_api_statuses.createStatus, {
    ...
    const statuses = await t.query(internal.admin_api_statuses.listStatuses, {
      organizationId: orgId,
    });
  ...
  test("updateStatus should update fields", async () => {
    ...
    const { id: statusId } = await t.mutation(
      internal.admin_api_statuses.createStatus,
      { organizationId: orgId, name: "Old", color: "#000" }
    );

    await t.mutation(internal.admin_api_statuses.updateStatus, {
      organizationId: orgId,
      statusId,
      name: "New",
      color: "#FFF",
  ...
  test("updateStatus should reject wrong org", async () => {
    ...
    await expect(
      t.mutation(internal.admin_api_statuses.updateStatus, {
        organizationId: otherOrgId,
        statusId,
        name: "Hacked",
      })
  ...
  test("deleteStatus should remove and clear feedback references", async () => {
    ...
    await t.mutation(internal.admin_api_statuses.deleteStatus, {
      organizationId: orgId,
      statusId,
    });
