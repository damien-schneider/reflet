...
function ContentRowSkeleton() {
  return (
    <div className="flex items-start gap-3 border-border border-b px-3 py-3 last:border-b-0">
...
export default function ContentPage() {
      ...
      <div className="space-y-4">
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          {Array.from({ length: 5 }, (_, i) => (
            <ContentRowSkeleton key={`skel-${String(i)}`} />
          ))}
        </div>
      </div>
    );
  ...
  const filteredDocs = contentDocs
    ...
    .sort((a, b) => {
      // Unread (needsReview) float to top
  ...
  return (
    <div className="flex h-48 items-center justify-center rounded-xl border border-dashed text-muted-foreground text-sm">
      <div className="text-center">
        ...
        {hasFilter ? (
          <p>
            No{platformLabel ? ` ${platformLabel}` : ""} content matching your
            filters
          </p>
        ) : (
          <>
            ...
            <p className="mt-1 text-xs">
              Agents will create social replies and blog posts here.
            </p>
