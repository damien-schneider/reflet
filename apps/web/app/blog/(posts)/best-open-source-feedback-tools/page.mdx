...
**Best for: Teams that want a full-featured, modern feedback platform they can self-host or use as a managed service.**

[Reflet](https://www.reflet.app) is a product feedback and roadmap tool built with Next.js, React 19, and Convex. It covers the complete feedback-to-roadmap workflow in a single, cohesive product.
...
- **Real-time collaboration**: Built on Convex, which provides true real-time synchronization. Changes appear instantly for all connected users without polling or page refreshes.
- **Custom branding**: Custom domain, custom CSS, and white-label branding are available without requiring an enterprise plan upgrade.
- **Team management**: Role-based access control (RBAC) for team members, with granular permissions.
- **Smart notifications**: Users are notified when feedback they care about changes status.
- **Full-text search**: Find feedback across all boards quickly.
- **Modern UX**: The interface reflects current design standards, not early 2010s patterns.
...
Reflet is newer than tools like Fider, so its community and ecosystem are still growing. It does not currently offer AI-powered feedback categorization or NPS surveys.

### Pricing

Reflet offers a free tier with core functionality, paid plans for growing teams, and a self-hosted option at no software cost.
...
Fider has no roadmap view and no changelog. If you need to communicate product status or announce releases, you will need a separate tool. The UI is functional but dated compared to more recently built alternatives. There are also no native integrations with project management tools like Jira or Linear.
...
Astuto's integrations are webhook-based rather than native, which means some setup is required for each connection. The roadmap and changelog are more basic than what you get in tools like Canny or Reflet. Real-time updates are limited.
...
| Tool | Feedback Board | Roadmap | Changelog | Self-Hostable | License | Best For |
|---|---|---|---|---|---|---|
| **Reflet** | Yes | Yes (Kanban) | Yes | Yes | SSPL | Full workflow, modern UX, real-time |
| **Fider** | Yes | No | No | Yes | AGPL | Simplicity, minimal infra |
| **Astuto** | Yes | Yes | Basic | Yes | Open source | Affordable hosted + roadmap |
| **Formbricks** | No | No | No | Yes | AGPL | Surveys and in-app research |
| **LogChimp** | Yes | No | No | Yes | GPL | Lightweight, developer-friendly |
| **ClearFlask** | Yes | Yes | Yes | Yes | Apache 2.0 | Feature-complete, complex setups |
| **Plane** | No (internal only) | Yes | No | Yes | AGPL | Dev teams, internal tracking |
...
**You want a full feedback-to-roadmap workflow with modern UX:** Reflet. It is the only tool in this list that covers feedback boards, kanban roadmap, and changelog in a cohesive real-time experience.
...
Reflet is one of the newer entrants in this space, but it is the one most focused on delivering the complete feedback workflow — boards, roadmap, changelog — with a modern real-time architecture and custom branding that does not require an enterprise upgrade.
