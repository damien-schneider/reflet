...
**Learning:** The application uses SSR (via TanStack Router/Start), so standard `dompurify` fails because it requires a browser environment (DOM) which is missing on the server.
